#!/usr/bin/env python3
"""
Gera o raster final do Deficit de Capacidade de Resposta (Indice V),
combinando as duas subdimensoes:

    Deficit_Resposta = RESPONSE_WEIGHT_ACCESSIBILITY * Deficit_Acessibilidade
                      + RESPONSE_WEIGHT_COVERAGE      * Deficit_Cobertura

Ambas as subdimensoes ja estao na mesma semantica (valor alto = pior,
ver scripts/gerar_deficit_acessibilidade.py e gerar_deficit_bombeiros.py),
pelo que a combinacao e simplesmente aditiva -- mesma logica ja usada na
Sensibilidade (Vulnerabilidade Demografica + Social).

Uso:
    python3 gerar_deficit_resposta.py <mascara.tif> <deficit_acessibilidade.tif> <deficit_bombeiros.tif> <output.tif>
"""
import sys
import os
import numpy as np
...
def exportar_histograma_texto(valores, path, n_bins=20, escala_max=1.0):
    ...
    with open(path, "w") as f:
        f.write(f"Histograma do Deficit de Capacidade de Resposta -- 0.0 a {escala_max:.4f}\n")
        f.write("=" * 70 + "\n")
...
def main():
    if len(sys.argv) != 5:
        print("Uso: python3 gerar_deficit_resposta.py <mascara.tif> <deficit_acessibilidade.tif> "
              "<deficit_bombeiros.tif> <output.tif>")
        sys.exit(1)

    mascara_path, acess_path, bombeiros_path, output_path = sys.argv[1:5]
    ...
    print(f"A ler deficit de acessibilidade de {acess_path} ...")
    deficit_acess, nodata_acess = ler_raster_completo(acess_path)

    print(f"A ler deficit de cobertura operacional de {bombeiros_path} ...")
    deficit_bomb, nodata_bomb = ler_raster_completo(bombeiros_path)

    print(f"A combinar: {cfg.RESPONSE_WEIGHT_ACCESSIBILITY} x Acessibilidade + "
          f"{cfg.RESPONSE_WEIGHT_COVERAGE} x Cobertura Operacional ...")
    deficit_resposta = (
        cfg.RESPONSE_WEIGHT_ACCESSIBILITY * deficit_acess
        + cfg.RESPONSE_WEIGHT_COVERAGE * deficit_bomb
    )

    deficit_final = np.where(dentro_pt, deficit_resposta, gs.GRID_NODATA)
    ...
    print("=== Estatisticas do raster final (Deficit de Capacidade de Resposta) ===")
